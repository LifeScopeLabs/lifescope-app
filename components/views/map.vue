<template>
    <div>
        ...
        <div v-if="$store.state.searching === true"
             class="map-corner flexbox"
        >
            <div>
        ...
        <div v-if="$store.state.searchEnded === false && $store.state.searching !== true"
             class="map-corner clickable"
             v-on:click="$root.$emit('perform-search', false)"
        >
		...
		methods: {
			...
			populateMap: function() {
				...
				function initializeSpiderLeg(spiderLeg) {
					...
					pinElem.innerHTML = '<i class="circle-icon fas fa-map-marker fa-stack-2-5x"></i>' + '<i class="type-icon ' + self.getEventTypeIcon(event.type) + ' fa-stack-1x"></i>';
					...
					$(pinElem)
						.on('mouseenter', function() {
							...
							popup.setHTML(event.context + ' via ' + event.connection.provider.name + ' on ' + moment.utc(event.datetime)
								.local()
								.format('M/D/YY')).addTo(map);

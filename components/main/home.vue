        ...
        <transition appear name="page-load">
            <aside v-if="$store.state.user != undefined" id="profile">
                <nuxt-link to="/settings/connections">
                    <div class="avatar">
                        <transition name="opacity" mode="out-in">
                            <img class="me"
                                 v-if="$store.state.person.avatar_url != null && $store.state.person.avatar_url.length > 0"
                                 v-bind:src="$store.state.person.avatar_url"
                                 style="max-height: 100px; max-width: 100px;">
                            <div class="default me"
                                 v-else-if="$store.state.person.avatar_url == null || $store.state.person.avatar_url.length === 0"
                ...
                <div id="stats-container">
                    <div class="scroller">
                        <div class="stats">
                            <nuxt-link class="connections flexbox flex-space-between" to="/settings/connections">
                                <div class="count" v-model="connectionCount">Connections</div>
                                <div class="count-val"> {{ connectionCount }}</div>
                            </nuxt-link>

                            <a class="people clickable flexbox flex-space-between" v-on:click="selectTab('people')">
                                <div class="count" v-model="personCount">People</div>
                                <div class="count-val"> {{ personCount }}</div>
                            </a>

                            <nuxt-link class="events flexbox flex-space-between" to="/explore">
                                <div class="count" v-model="eventCount">Events</div>
                                <div class="count-val"> {{ eventCount }}</div>
                            </nuxt-link>

                            <nuxt-link class="content flexbox flex-space-between" to="/explore?facet=content">
                                <div class="count" v-model="contentCount">Content</div>
                                <div class="count-val"> {{ contentCount }}</div>
                            </nuxt-link>

                            <nuxt-link class="contacts flexbox flex-space-between" to="/explore?facet=contacts">
                                <div class="count" v-model="contactCount">Contacts</div>
                                <div class="count-val"> {{ contactCount }}</div>
                            </nuxt-link>

                            <nuxt-link class="locations flexbox flex-space-between" to="/settings/locations">
                                <div class="count" v-model="locationCount">Locations</div>
                            ...
                            <div class="searches clickable flexbox flex-space-between"
                                ...
                                <div class="count" v-model="$store.state.searchCount">Searches</div>
        ...
        <transition appear name="page-load">
            <section v-if="$store.state.user != undefined" id="content">
                <div class="flexbox">
                    <transition appear name="page-load">
                        <nav id="tabs">
                            <div class="tab" name="people" v-bind:class="{selected: $store.state.home.tab === 'people'}"
                                 v-on:click="fetchData(true, 'people', 'first_name')">People
                            </div>
                            <div class="tab" name="favorited"
                                 v-bind:class="{selected: $store.state.home.tab === 'searches'}"
                                 v-on:click="fetchData(true, 'searches', 'favorited')">Searches
                            </div>
                            <div class="tab" name="tags" v-bind:class="{selected: $store.state.home.tab === 'tags'}"
                                 v-on:click="fetchData(true, 'tags', 'tag')">Tags
                            </div>

                            <transition name="opacity">
                                <div id="sort" class="flexbox flex-grow flex-end"
                                     v-if="$store.state.home.tab === 'people'">
                                    <div v-bind:class="{selected: $store.state.home.sort === 'first_name'}"
                                         v-on:click="fetchData(true, 'people', 'first_name')">First
                                    </div>
                                    <div v-bind:class="{selected: $store.state.home.sort === 'middle_name'}"
                                         v-on:click="fetchData(true, 'people', 'middle_name')">Middle
                                    </div>
                                    <div v-bind:class="{selected: $store.state.home.sort === 'last_name'}"
                                         v-on:click="fetchData(true, 'people', 'last_name')">Last
                                    </div>
                                </div>
                            </transition>
                            ...
                            <transition name="opacity">
                                <div id="sort" class="flexbox flex-grow flex-end"
                                     v-if="$store.state.home.tab === 'searches'">
                                    <div v-bind:class="{selected: $store.state.home.sort === 'favorited'}"
                                         v-on:click="fetchData(true, 'searches', 'favorited')">Favorited
                                    </div>
                                    <div v-bind:class="{selected: $store.state.home.sort === 'top'}"
                                         v-on:click="fetchData(true, 'searches', 'top')">Top
                                    </div>
                                    <div v-bind:class="{selected: $store.state.home.sort === 'recent'}"
                                         v-on:click="fetchData(true, 'searches', 'recent')">Recent
                                    </div>
                                </div>
                            </transition>
                            ...
                            <transition name="opacity">
                                <div id="sort" class="flexbox flex-grow flex-end"
                                     v-if="$store.state.home.tab === 'tags'">
                                    <div v-bind:class="{selected: $store.state.home.sort === 'tag'}"
                                         v-on:click="fetchData(true, 'tags', 'tag')">A-Z
                                    </div>
                                    <div v-bind:class="{selected: $store.state.home.sort === 'shared'}"
                                         v-on:click="fetchData(true, 'tags', 'shared')">Shared
                                    </div>
                                </div>
                            </transition>
                        </nav>
                ...
                <div id="search-container" v-on:scroll="handleScroll">
                    <div class="scroller">
                        <div id="searches">
                            <transition-group appear name="search-item" tag="p">
                                <nuxt-link v-if="$store.state.home.tab === 'searches'"
                                           v-for="search in $store.state.searchMany" v-bind:to="constructLink(search)"
                                           class="saved-search"
                                    ...
                                    <div class="info">
                                        <i v-bind:class="searchIcon(search)"
                                           v-bind:style="{ color: searchColor(search) }"></i>
                                    ...
                                    <div v-if="search.query || (search.filters && search.filters.length > 0)"
                                        ...
                                        <div v-if="search.filters && search.filters.length > 0" class="filters">
                                            <div v-for="filter in search.filters" class="filter">{{ filter.name ||
                                                filter.type }}
                                            </div>
                                    ...
                                    <div v-bind:class="favoriteButton(search)"
                                         v-on:click.stop.prevent="showFavoriteModal(search)"></div>
                                </nuxt-link>

                                <a v-if="$store.state.home.tab === 'tags'" v-for="tag in $store.state.tagMany"
                                   class="tag" v-on:click="searchForTag(tag.tag)" v-bind:key="tag.id">
                                    <div>
                                ...
                                <a v-if="$store.state.home.tab === 'people'" v-for="person in $store.state.personMany"
                                   class="person" v-on:click="searchForPerson(person)" v-bind:key="person.id">
                                    <div>
                                        <div class="avatar">
                                            <img v-if="person.avatar_url != null && person.avatar_url.length > 0"
                                                 v-bind:src="person.avatar_url">
                                            <div class="default"
                                                 v-else-if="person.avatar_url == null || person.avatar_url.length === 0"
                            ...
                            <transition name="opacity">
                                <div id="more-people" class="flexbox flex-center"
                                     v-if="$store.state.home.tab === 'people'" v-on:click="redirectToPeople">

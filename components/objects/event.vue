    ...
    >
        ...
        >
            ...
            >
                ...
                <!-- content -->
                <section v-if="event.content && event.content.length > 0"
                         class="content"
                >
                    <user-content v-for="content in event.content"
                                  v-bind:key="content.id"
                                  v-bind:content="content"
                                  v-bind:connection="event.connection"
                    ></user-content>
                </section>

                <!-- interactions -->
                <aside v-if="(event.contacts && event.contacts.length > 0) || (event.people && event.people.length > 0) || (event.organizations && event.organizations.length > 0)"
                       class="interactions"
                >
            ...
            <div v-else-if="event.hidden === true"
                 class="event-hidden"
            >
                <div class="unhide-button"
                     v-on:click="unhideEvent(event)"
                >
        ...
        >
            ...
            <i v-else
               v-bind:class="getEventTypeIcon(event.type)"
            ...
            <div v-if="event.hidden !== true"
                 class="title-bar"
            >
                <i v-bind:class="getEventTypeIcon(event.type)"
                   class="bubble"
                ></i>

                <div v-if="hasTitle() === true"
                     class="title"
                >
                    {{ getGridTitle() | safe }}
                </div>
                ...
                <i v-bind:class="getProviderIcon(event.connection.provider)"
                   class="bubble"
                ></i>
            </div>
        </div>
        ...
        >
            ...
            <div class="icon-column">
                <i v-if="event.hidden !== true"
                   v-bind:class="getEventTypeIcon(event.type)"
                ></i>
                <span v-if="event.hidden !== true"
                      class="mobile-hide"
                >{{ contextOrType(event) | truncate(30) }}</span>
            </div>

            <div class="icon-column">
                <i v-if="event.hidden !== true"
                   v-bind:class="getProviderIcon(event.connection.provider)"
                ></i>
                <span v-if="event.hidden !== true"
                      class="mobile-hide"
                >{{ event.connection.provider.name }}</span>
            </div>

<template>
	<!-- feed -->
		<transition name="event" mode="out-in">
			<div v-bind:key="1" v-if="$store.state.view === 'feed'" class="object event feed" v-bind:id="event.id">
		...
		<!-- grid -->
			<div v-bind:key="2" v-else-if="$store.state.view === 'grid'" class="item grid" v-bind:id="event.id"
		...
		<!-- list -->
			<div v-bind:key="3" v-else-if="$store.state.view === 'list'" class="item list" v-bind:id="event.id"
				 v-on:click="$emit('render-details', event, 'event')">
				<div>

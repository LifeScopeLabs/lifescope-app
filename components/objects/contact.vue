<template>
    <transition name="contact" mode="out-in">
        ...
        <!-- grid -->
        <div v-bind:key="2" v-else-if="$store.state.view === 'grid'" class="item grid" v-bind:id="contact.id"
        ...
        <!-- list -->
        <div v-bind:key="3" v-else-if="$store.state.view === 'list'" class="item list" v-bind:id="contact.id"
             v-on:click="$emit('render-details', contact, 'contact')">
            <div>
